...
where
    I2C: I2c,
{
    pub fn new(i2c: I2C, default_addr: bool) -> Self {
        BNO08x {
    ...
    pub fn read_packet(&mut self) -> Packet {
        ...
        self.i2c.read(self.address, out.as_mut_data());

        info!("FULL PACKET {}", out.full_packet().as_slice());
        out
    }
    ...
    pub fn send_packet(&mut self, channel: u8, data: &[u8]) {
        ...
        self.i2c.write(self.address, write.full_packet().as_slice());
        info!("PACKET SENT");
    }
